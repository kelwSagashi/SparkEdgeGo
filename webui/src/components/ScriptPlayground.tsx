...
export default function ScriptPlayground({
  ...
  const handleInputChange = (
    ...
    setInputs((prev) => ({
      ...prev,
      [name]: type === "number" ? (value === "" ? "" : valueAsNumber) : value,
    }));
  };
      ...
      <div className="w-1/2 p-4 border-r border-white/[0.08] flex flex-col gap-4 overflow-y-auto">
        ...
        {loading && !schema ? (
          <div className="flex justify-center p-4">
            <div className="w-4 h-4 border-2 animate-spin rounded-full border-t-transparent" />
          </div>
        ) : (
                ...
                <div key={inp.name} className="flex flex-col gap-1.5">
                  ...
                  {inp.type === "boolean" ? (
                    <Checkbox
                      ...
                      onCheckedChange={(v) =>
                        setInputs((prev) => ({ ...prev, [inp.name]: !!v }))
                      }
                    />
                  ) : (
                    <Input
