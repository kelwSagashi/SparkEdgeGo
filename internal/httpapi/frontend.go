...
func resolveFrontendSource() (frontendSource, error) {
	candidates := []string{}
	if env := strings.TrimSpace(os.Getenv("SPARKEDGE_WEBUI_DIST")); env != "" {
	...
	if cwd, err := os.Getwd(); err == nil {
		candidates = append(candidates,
			filepath.Join(cwd, "webui", "dist"),
			filepath.Join(cwd, "frontend", "dist"),
		)
	}
	candidates = append(candidates,
		appfs.ResolveFromRoot("webui", "dist"),
		appfs.ResolveFromRoot("frontend", "dist"),
	)
	...
	return nil, NewHTTPError(http.StatusServiceUnavailable, "webui assets not found; provide webui/dist next to the executable or configure SPARKEDGE_WEBUI_DIST")
}
